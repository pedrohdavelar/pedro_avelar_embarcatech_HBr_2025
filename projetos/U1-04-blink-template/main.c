...
como a leitura da temperatura depende do ADC, vou deixar o código para ler a temperatura junto do código do joystick
depois preciso 1) dar uma limpada e deixar um projeto padrão 2) adicionar toda a tabela ASCII ao header do display oled
 */
...
void initJoystick(){
    ...
    //código para leitura de temperatura
    adc_set_temp_sensor_enabled(true);
}

//pausa no codigo do joystick para colocar o codigo para leitura de temperatura
float readTemperature(){
    adc_select_input(4); // Seleciona o canal ADC 4 para leitura da temperatura 
    uint16_t adcValue = adc_read();
    const float conversionFactor = 3.3f / (1 << 12); // Conversão de ADC para tensão (12 bits)
...
void playStartupTune(){
    ...
    int noteDuration = 66;
    for (int i = 0; i < 3; ++i){
    play_tone(2000 + i*500, noteDuration);
    play_tone(1500 + i*500, noteDuration);
    play_tone(1000 + i*500, noteDuration);
    play_tone(3000 + i*500, noteDuration);
    play_tone(2000 + i*500, noteDuration);
    play_tone(1000 + i*500, noteDuration);
    play_tone(5000 + i*500, noteDuration);
    play_tone(2500 + i*500, noteDuration);
    play_tone(1250 + i*500, noteDuration);
    play_tone(2000 + i*500, noteDuration);
    }
}
...
int main() {
    ...
    ssd1306_draw_string(ssd, 0,0,   "Leitura da      ");
    ssd1306_draw_string(ssd, 0,8,   "     Temperatura");
    ...
    //variaveis 
    float rp2040Temp = 0.0f; //temperatura do rp2040 atual
    float rp2040TempAvg = 0.0f; //temperatura do rp2040 média
    float rp2040TempMax = 0.0f;
    float rp2040TempMin = 0.0f;
    char rp2040TempChar[10]; //string para guardar a temperatura do rp2040
    char rp2040TempMaxChar[10];
    char rp2040TempMinChar[10];
    char dotCount[11]; // string para armazenar até 10 pontos para mostrar o avanço do tempo (deixando em 10 estava imprimindo lixo no 10 ponto, por isso deixei 11 =) )
    
    //fazemos uma primeira leitura antes de entrar no loop
    rp2040Temp = readTemperature();
    rp2040TempAvg = rp2040Temp; 
    rp2040TempMax = rp2040Temp; 
    rp2040TempMin = rp2040Temp; 
    sprintf(rp2040TempChar, "%.2f", rp2040Temp); 
    sprintf(rp2040TempMaxChar, "%.2f", rp2040TempMax); 
    sprintf(rp2040TempMinChar, "%.2f", rp2040TempMin); 
    
    while (true) {

        //a temperatura está oscilando muito então irei pegar 10 medidas com um intervalo de 100ms e atualizar a temperatura a cada segundo
        rp2040Temp = 0.0f;
        memset(dotCount, 0, sizeof(dotCount));
        for (int i = 0; i < 10; ++i){
            rp2040Temp += readTemperature(); 
            dotCount[i] = '.';
            memset(ssd, 0, ssd1306_buffer_length);
            ssd1306_draw_string(ssd, 0 ,0,  "Temperatura:");
            ssd1306_draw_string(ssd, 0 ,8,  dotCount);
            ssd1306_draw_string(ssd, 0 ,24, "Atual ");
            ssd1306_draw_string(ssd, 48 ,24, rp2040TempChar);
            ssd1306_draw_string(ssd, 0 ,32, "Min ");
            ssd1306_draw_string(ssd, 48 ,32, rp2040TempMinChar);
            ssd1306_draw_string(ssd, 0 ,40, "Max ");
            ssd1306_draw_string(ssd, 48 ,40, rp2040TempMaxChar);
            render_on_display(ssd, &frame_area); 
            printf(dotCount);
            sleep_ms(100);
        }   
        rp2040TempAvg = rp2040Temp/10.0f; //faz a média das 10 leituras
        printf("Temperatura do RP2040: %.2f C\n", rp2040Temp); //print no console para debug
        
        sprintf(rp2040TempChar, "%.2f", rp2040TempAvg); //converte a temperatura para string

        if (rp2040TempAvg > rp2040TempMax){ //atualiza a temperatura máxima
            rp2040TempMax = rp2040TempAvg;
            sprintf(rp2040TempMaxChar, "%.2f", rp2040TempMax); 
        } else if (rp2040TempAvg < rp2040TempMin){ //atualiza a temperatura mínima
            rp2040TempMin = rp2040TempAvg;
            sprintf(rp2040TempMinChar, "%.2f", rp2040TempMin); 
        }
