...
int main() {
    ...
    ssd1306_draw_string(ssd, 0,0,   "Leitura do      ");
    ssd1306_draw_string(ssd, 0,8,   "        Joystick");
    ...
    while (true) {
        ...
        readJoystick();
        sprintf(joyXChar, "%d", joystick.x);
        sprintf(joyYChar, "%d", joystick.y);     


        memset(ssd, 0, ssd1306_buffer_length);
        ssd1306_draw_string(ssd, 0 ,0,  "Eixo X:");
        ssd1306_draw_string(ssd, 80,0,  joyXChar);
        ssd1306_draw_string(ssd, 0 ,24, "Eixo Y:");
        ssd1306_draw_string(ssd, 80,24, joyYChar);
        render_on_display(ssd, &frame_area); 

        if (joystick.x + joystick.y > 0){
            play_tone((joystick.x + joystick.y)*10, 50);
        } else if (joystick.x + joystick.y < 0){
            play_tone((-joystick.x - joystick.y)*10, 50);
        }
    } 
    return 0;
